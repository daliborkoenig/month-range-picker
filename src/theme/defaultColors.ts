...
/**
 * Get background color for month tile based on its state
 */
export const getMonthTileBackground = ({
  selected,
  inRange,
  hovered,
  disabled,
  ...
  if (inRange || hovered) {
    return manipulateColor(themeColors.secondary, 80);
  }
  return themeColors.text_inverted;
};

/**
 * Get text color for month tile based on its state
 */
export const getMonthTileTextColor = ({
  selected,
  inRange,
  hovered,
  disabled,
  ...
  if (disabled) {
    return manipulateColor(themeColors.text_inverted, 120);
  }
  if (selected || inRange || hovered) {
    return themeColors.text_inverted;
  }
